...
```shell
docker run -it --env-file $(pwd)/ta_env_vars.env -v $(pwd):/keys -v EVENT_LOG_PATH_ENTERED_DURING_INSTALLATION:/events axway.jfrog.io/ampc-public-docker-release/agent/v7-traceability-agent:latest
```
